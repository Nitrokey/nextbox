...
const createReport = (jobs) => {
    const insights = get_1.default(jobs, '[0].insights');
    const summary = config_1.SOURCE_PATHS.reduce((agg, sourceId) => {
        const sourceSummary = get_1.default(jobs, [0, 'summary', sourceId]);
        if (!sourceSummary) {
            return agg;
        }
        const output = Object.entries(sourceSummary).map(([metricId, summaryData]) => {
            const metric = metrics_1.getGlobalMetricType(`${sourceId}.${metricId}`);
            const { current = 0, baseline = 0 } = summaryData;
            const info = metrics_1.getMetricRunInfo(metric, current, baseline);
            return {
                label: metric.label,
                ...info,
            };
    ...
    return {
        ...
        summary,
        ...!isEmpty_1.default(insights) ? { insights } : {},
