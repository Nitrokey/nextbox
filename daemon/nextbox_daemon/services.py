...
class Services:
    ...
    * services control is either: `SAFE_SERVICE_CTRL` or `FULL_SERVICE_CTRL`
    ...
    FULL_SERVICE_CTRL = SAFE_SERVICE_CTRL + ["stop", "disable", "enable", "mask"]
    ...
    SERVICES_CTRL = {
        ...
        "userconfig": ("userconfig.service", FULL_SERVICE_CTRL),
    }

    # shortcut methods
    def status(self, name):
        return self.exec(name, "status")

    def start(self, name):
        return self.exec(name, "start")

    def stop(self, name):
        return self.exec(name, "stop")

    def restart(self, name):
        return self.exec(name, "restart")

    def enable(self, name):
        return self.exec(name, "enable")

    def disable(self, name):
        return self.exec(name, "disable")

    def unmask(self, name):
        return self.exec(name, "unmask")

    def mask(self, name):
        return self.exec(name, "mask")

    # special treatment, just want a boolean here...
    def is_active(self, name):
        return self.exec(name, "is-active").get("output", " ")[0].strip() == "active"

    def check(self, name, op):
        """Check, if given service (name) is allowed to run op(eration)"""

        return name in self.SERVICES_CTRL and op in self.SERVICES_CTRL[name][1]
